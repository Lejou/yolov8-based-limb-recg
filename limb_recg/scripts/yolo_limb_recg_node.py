...
class LimbSuctionPoints:
	def __init__(self):
		...
		self.yolo_model_ = YOLO('/home/z/yolov8/limb_yolo_reg/model/arm_seg3.pt', task='segment')
	...
	def getDepthFigure(self,skin_mask):
		...
		if self.depth_image_lock_:
			# self.rgb_image_obtained_ = copy.deepcopy(self.rgb_image_) 
			image_order = np.zeros(self.rgb_image_obtained_.shape[:2], np.int8)
			...
			try:
				...
				for i in range(self.depth_img_acquire_nums_):
					cv_img = self.bridge_.imgmsg_to_cv2(self.depth_image_array_[i] , self.depth_image_array_[0].encoding)
					if i==0:
						cv_depth_img_sum = cv_img
					else:
						cv_depth_img_sum += cv_img
					_, skin1 = cv2.threshold(cv_img/1000, 0.2, 1, cv2.THRESH_BINARY) 
					# _, skin2 = cv2.threshold(cv_img/1000, 0.6, 255, cv2.THRESH_BINARY) 
					# img_temp = skin1 - skin2 
					img_temp = skin1
					img_temp = img_temp.astype(np.int8)
					# print("img_temp: ",img_temp)
					image_order +=img_temp

					# cv2.imshow('depth skin1',skin1)
					# cv2.imshow('depth skin2',skin2)
					# cv2.waitKey(0)

				_, image_zero = cv2.threshold(image_order/self.depth_img_acquire_nums_, 0.05, 1, cv2.THRESH_BINARY_INV) 
				cv2.imshow('depth image_zero',image_zero)
				cv2.waitKey(0)
				# print("img zero: ",np.argwhere(image_order==0))

				image_order = image_order + image_zero
				
				cv_depth_img = cv_depth_img_sum / image_order
	...
	def skinImageProcess(self,img,skin_color_img):

		gray = cv2.cvtColor(skin_color_img, cv2.COLOR_BGR2GRAY)
		...
		# cv2.imshow("YOLOv8 img_erode2", img_erode2)
		...
		cv2.rectangle(img, (roi[0], roi[1]), (roi[0] + roi[2], roi[1] + roi[3]), (255,255,255), 2)

		# cv2.imshow('YOLOv8 img with roi:',img)
		# cv2.waitKey(0)
		# cv2.imshow('YOLOv8 limb_color_img',limb_color_img)
		# cv2.waitKey(30)
	...
	# 2: wholeArm
	# 3: arm (Generate)
	def skinDetectFromYolo(self, color_img, target_num):
		# results = model.predict(cv_img, save=False, imgsz=640, conf=0.75, visualize = False, show_labels=True, show_conf=False,show_boxes=True)	
		results = self.yolo_model_(color_img)
		
		annotated_frame = results[0].plot(boxes=True)
		# cv2.imshow("YOLOv8 Inference", annotated_frame)

		label_num_arr = np.array(results[0].boxes.cls.tolist())
		label_num_arr = label_num_arr.astype(np.int8)

		if target_num in label_num_arr:
			# index_list = np.argwhere(label_num_arr == target_num)
			index = np.array(label_num_arr.tolist().index(target_num)) # obtain the index with the the large conf
			# print("index_list: ", index_list)

			b_mask = np.zeros(color_img.shape[:2], np.uint8)
			#  Extract contour result
			# contour = c.masks.xy.pop()
			contour = results[0].masks.xy[index]
			# print("find label index: ", contour)
			#  Changing the type
			...
			skin_mask = cv2.drawContours(b_mask,
								[contour],
								-1,
								(255, 255, 255),
								cv2.FILLED)

			limb_color_img = cv2.bitwise_and(color_img,color_img, mask = skin_mask)
			# cv2.imshow('YOLOv8 skin image',limb_color_img)
			# cv2.waitKey(30)

		return self.skinImageProcess(color_img,limb_color_img)
	...
	def getPonitCloud4(self, img_roi, suction_points_3d,suction_point_2d,norms_construction_2d_point):
		...
		for j in range(roi_x, roi_x + roi_w):  #height
			for i in range(roi_y, roi_y + roi_h):  #width
				...
				if z > 200 and z < 600:  # 忽略深度为0和大于600 mm的的点 
					point_tmp = rs2.rs2_deproject_pixel_to_point(self.camera_color_intrinsics_, [x, y], z)
		...
		pcd = o3d.geometry.PointCloud()  
		pcd.points = o3d.utility.Vector3dVector(array_points_roi[:,:3]) 
		filter_pcd,_ = pcd.remove_statistical_outlier(30,0.01)
		...
		for num in range(array_suction_points_3d.shape[0]):
			...
			print("norm_x : ",num, norm_x)
			print("norm_x 1: ", norm_3d_pts_mean_array[4*num+1,:])
			print("norm_x 0: ", norm_3d_pts_mean_array[4*num+0,:])
			# print("norm_y : ",norm_y)
	...
	def getSuctionRoute(self,img_roi,rgb_gray):
		...
		# 计算B样条曲线的节点和系数
		self.suction_route_fit_line_ = UnivariateSpline(points_control[:,0], points_control[:,1], k=3, s=0)
		...
		# cv2.imshow('route color image',color_image)
		# cv2.waitKey(30)
		...
		for point2d in route_2d:
			if point2d[0] < 840 and point2d[1] < 480 :
				...
				if z > 200 and z < 600: # unit:mm
					...
					if np.sqrt(np.sum((np.array([point_3d[:2]]) - np.array(anchor_pose[:2])) ** 2)) >= dis:
					# if np.sqrt(np.sum((np.array([point_3d[0:1]]) - np.array(anchor_pose[0:1])) ** 2)) >= dis:
						if num >10 and num < len(route_2d)-40:
							anchor_num = num
	...
	def colorImgYoloProcess(self,target_num):
		# if self.camera_color_intrinsics_:
		#		 return
		rgb_image = self.getColorFigure()

		self.rgb_image_obtained_, roi, recog_mask, rgb_gray_image = self.skinDetectFromYolo(rgb_image,target_num)
...
def main():
	...
	try:
		while not rospy.is_shutdown():
			 rate.sleep() 
			 limb_suction_detec.colorImgYoloProcess(1)
			 input()
		...
		# 				suction_poses_msg = limb_suction_detec.colorImgYoloProcess(1)
