...
class MoveGroupPanda(object):
	...
	def go_to_muti_line_movement(self,muti_point,state):
		...
		plan_temp=copy.deepcopy(plan)
		new_plan= self.scale_trajectory_speed(plan_temp, self.line_velocity_scale)
		return move_group.execute(new_plan, wait=state)    
...
def main():
	...
	# dr = data_record()
	phantom_pose_base = [
		0.548, -0.02, 0.056, 0.7358611627835119, -0.6769614389105694, 0.014871272691383733, 0.0025685217170779316
	  ]

	# pneumatic.uuid = roslaunch.rlutil.get_or_generate_uuid(None, False)
	# roslaunch.configure_logging(pneumatic.uuid)
	# pneumatic.launch_start_force = roslaunch.parent.ROSLaunchParent(pneumatic.uuid, ["/home/robot/a_panda_ws/src/franka_controllers/franka_effort_controller/launch/start_force_control.launch"])
	# pneumatic.launch_stop_force = roslaunch.parent.ROSLaunchParent(pneumatic.uuid, ["/home/robot/a_panda_ws/src/franka_controllers/franka_effort_controller/launch/stop_force_control.launch"])
	...
	# print("target_waypoints",target_waypoints)
	...
	try:
		...
		target_waypoints = []
		phantom_pose_start = copy.deepcopy(phantom_pose_base)
		phantom_pose_start[2] = phantom_pose_base[2]+0.05
		target_waypoints.append(copy.deepcopy(phantom_pose_start))
		pneumatic.panda.set_line_velocity_scale(0.1)
		pneumatic.panda.go_to_muti_line_movement(target_waypoints,True)

		input("============ Press `Enter` to close to the suction point ...")
		target_waypoints = []
		target_waypoints.append(copy.deepcopy(phantom_pose_test))
		pneumatic.panda.set_line_velocity_scale(0.1)
		pneumatic.panda.go_to_muti_line_movement(target_waypoints,True)
		print("Turn on the main pressure!")
		pneumatic.turn_on_main_pressure(255,160)
		print("Turn on the lip pressure!")
		pneumatic.turn_on_lip_with_limit_pressure(-20)
		print("align the base pressure!")
		pneumatic.turn_on_recording()

		input("============ Press `Enter` to move to the suction point ...")
		target_waypoints = []
		# phantom_pose_base[2] = phantom_pose_base[2]+0.002
		# phantom_pose_base[2] = phantom_pose_base[2]+0.001
		phantom_pose_base[2] = phantom_pose_base[2]-0.002
		# phantom_pose_base[0] = phantom_pose_base[0]-0.002
		target_waypoints.append(copy.deepcopy(phantom_pose_base))
		...
		input("============ Press `Enter` to start the force controller ...")
		if pneumatic.mutex.acquire(False):
			pneumatic.launch_start_force = roslaunch.parent.ROSLaunchParent(pneumatic.uuid, ["/home/robot/a_panda_ws/src/franka_controllers/franka_effort_controller/launch/start_force_control.launch"])

			pneumatic.launch_start_force.start()
		pneumatic.mutex.release()
		sleep(2)

		input("============ Press `Enter` to stop the force controller ...")
		print("Turn off the main pressure!")
		pneumatic.turn_off_main_pressure()
		print("Turn off the lip pressure!")
		pneumatic.turn_off_lip()
		if pneumatic.mutex.acquire(False):
			# pneumatic.launch_start_force.shutdown()
			pneumatic.launch_stop_force = roslaunch.parent.ROSLaunchParent(pneumatic.uuid, ["/home/robot/a_panda_ws/src/franka_controllers/franka_effort_controller/launch/stop_force_control.launch"])
			pneumatic.launch_stop_force.start()
		pneumatic.mutex.release()
		sleep(2)
		print("align the base pressure!")
		pneumatic.turn_off_recording()
