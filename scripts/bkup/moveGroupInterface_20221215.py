...
class ArduinoPneumaticActuator(object):
	...
	def __init__(self):
		...
		self.main_value_limit_factor_ = 0.75
		self.main_valve_limit_ = 15  # negative pressure with positive value
		self.main_valve_trigger_pressure_ = self.main_valve_limit_*self.main_value_limit_factor_
...
def main():
	...
	try:
		...
		if(trajectory_state):
			while(pneumatic.trajectory_state_ == 1):
				if(pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_):
					...
					pressure_state = True
					break
				current_pose = pneumatic.panda.move_group.get_current_pose().pose
				if(all_close(wpose,current_pose,0.01)):
					# print("wpose",wpose)	
					# print("current_pose",current_pose)	
					...
					if(pneumatic.trajectory_state_ == 3):
						...
						position_state = False
						running_state = True
						print("Trajectory finished with wrong position. Stop!")
						print("wpose",wpose)	
						print("current_pose",current_pose)
				...
				if(pneumatic.get_main_pressure_value() < 20):
					...
					while(not pressure_meassure_flag):
						if(pressure_cnt<50):
							...
							pressure_state = pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_
			if(not pressure_state):
				print("!!!!!!!!!!!!Massage not working after trajectory, stop this point!")
			else:
