...
class ArduinoPneumaticActuator(object):
	...
	def excute_massage(self,target_pose,interval_time):

		waypoints = []
		ready_grasp_pose = self.find_neighbor_pose(-0.02, target_pose)
		deep_grasp_pose = self.find_neighbor_pose(0.005, target_pose)

		waypoints.append(copy.deepcopy(ready_grasp_pose))
		# print(waypoints)
		self.trajectory_state_ = 1
		ready_pose_position_state = self.panda.go_to_waypoints(waypoints,True)	
		...
		waypoints = []
		waypoints.append(copy.deepcopy(deep_grasp_pose))
		# print(waypoints)
		self.trajectory_state_ = 1
		trajectory_state = self.panda.go_to_waypoints(waypoints,False)
...
def main():
	...
	wpose.position.y = -0.1367
	wpose.position.z = 0.1260
	...
	# ready_grasp_pose = copy.deepcopy(wpose)
	# trans_matrix_cur_pose = pneumatic.ros_pose_to_transform_matrix(wpose)
	# trans_matrix_ready_pose = copy.deepcopy(trans_matrix_cur_pose)

	# ready_grasp_distance =numpy.array([[0,0,-0.02,1]]).T 
	...
	# ready_grasp_pose = pneumatic.transform_matrix_to_ros_pose(trans_matrix_ready_pose)
	# print("ready_grasp_pose ", ready_grasp_pose)

	ready_grasp_pose = pneumatic.find_neighbor_pose(-0.02, wpose)

	wpose.position.x = 0.6490
	wpose.position.y = -0.061
	wpose.position.z = 0.1260
	wpose.orientation.x=1
	wpose.orientation.y=0
	wpose.orientation.z=0
	...
	try:
		...
		input(
			"============ 2 Press `Enter` to massage pose 1 step 1 ..."
		)

		target_pose = copy.deepcopy(target_waypoints[0])
		pneumatic.excute_massage(target_pose, interval_time_)

		# waypoints = []
		# # wpose = copy.deepcopy(ready_grasp_pose)
		# # wpose.position.z = 0.146
		# waypoints.append(copy.deepcopy(ready_grasp_pose))
		# # print(waypoints)
		# pneumatic.trajectory_state_ = 1
		# pneumatic.panda.go_to_waypoints(waypoints,True)	
		# pneumatic.turn_on_main_pressure(250,250)

		# input(
		# 	"============ 3 Press `Enter` to massage pose 1 step 2..."
		# )
		# waypoints = []
		# wpose = copy.deepcopy(target_waypoints[0])
		# wpose.position.z = 0.106
		# waypoints.append(copy.deepcopy(wpose))
		# # print(waypoints)
		# pneumatic.trajectory_state_ = 1
		# trajectory_state = pneumatic.panda.go_to_waypoints(waypoints,False)
		# position_state = False
		# pressure_state = False
		# running_state = not pneumatic.trajectory_state_ == 1

		# if(trajectory_state): # planning check
		# 	print("pneumatic.trajectory_state_ ",pneumatic.trajectory_state_)	

		# 	while(pneumatic.trajectory_state_ < 2): # trajectory running state
				
		# 		if(pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_):
		# 			print("!!!!!!!!!!!!Pressure Limit!")
		# 			pneumatic.panda.move_group.stop()
		# 			# running_state = True
		# 			position_state = True
		# 			pressure_state = True
		# 			break
		# 	if(pneumatic.trajectory_state_ == 3):
		# 		position_state = True
		# 		pressure_state = False
		# 		print("Trajectory finished with no pressure!")
		# 		# print("wpose",wpose)	
		# 		# print("current_pose",current_pose)
		# 	elif(pneumatic.trajectory_state_ == 4):
		# 		print("Trajectory Aborted!")
		# 		position_state = False
		# 		pressure_state = False
		# 	elif(not pneumatic.trajectory_state_ == 1):
		# 		print("Trajectory running problems. pneumatic.trajectory_state_ ",pneumatic.trajectory_state_)	
				
		# 	if(not pressure_state and position_state):
		# 		print("!!!!!!!!!!!!Massage not working during trajectory!")
		# 		pressure_cnt = 0
		# 		if(pneumatic.get_main_pressure_value() < 20):
		# 			pressure_state = False
		# 			print("!!!!!!!!!!!!Low pressure. No hope for massage!")

		# 		else:
		# 			pressure_meassure_flag = pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_
		# 			while(not pressure_meassure_flag):
		# 				if(pressure_cnt<50):
		# 					pneumatic.interval_time(0.1)
		# 					pressure_cnt =pressure_cnt+1
		# 					pressure_state = pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_
		# 					pressure_meassure_flag = pressure_state
		# 				else:
		# 					pressure_meassure_flag = True
		# 					pressure_state = pneumatic.get_main_pressure_value() >= pneumatic.main_valve_trigger_pressure_
		# 	if(pressure_state):
		# 		print("!!!!!!!!!!!!Massage is working!")
		# 		pneumatic.interval_time(interval_time_)
		# 	pneumatic.turn_off_main_pressure()
		# else:
		# 	print("!!!!!!!!!!!!trajectory planning failed!")
			

		# print("============ Pressure massage demo complete!")
