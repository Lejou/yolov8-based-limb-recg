...
class MoveGroupPanda(object):
	...
	def __init__(self):
		...
		move_group.set_end_effector_link("probe_ee")
...
class ArduinoPneumaticActuator(object):
	...
	def main_pressure_value_callback(self,data):
		# print(-data.data * 7.5)
		self.main_pressure = int(-data.data * 7.5)
		# self.main_pressure = math.ceil(-data.data * 7.5)

	def turn_on_main_pressure(self,main_pwm,main_valve_limit):
		self.main_pwm = int(main_pwm)
	...
	def turn_off_main_pressure(self):
		...
		pwm_msg = Int16()
		pwm_msg.data = self.main_pwm
		self.main_pwm_pub.publish(pwm_msg)

	def interval_time(self,time):
		print("Interval time")
		sleep(time)
...
def main():
	...
	panda.set_joint_velocity_scale(0.2)
	panda.set_joint_acceleration_scale(0.2)
	panda.set_line_velocity_scale(0.2)
	...
	while(1):
		try:
			...
			# probe_ee
			# wpose.position.x = 0.424
			# wpose.position.y = -0.016
			# wpose.position.z = 0.373
			# waypoints.append(copy.deepcopy(wpose))
			...
			# waypoints.append(copy.deepcopy(wpose))
			# panda.go_to_waypoints(waypoints)
			# pneumatic.turn_on_main_pressure(255,250)
			# pneumatic.interval_time(interval_time_)
			# pneumatic.turn_off_main_pressure()
